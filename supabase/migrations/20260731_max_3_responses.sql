-- Sänk max antal betalda svar per cykelärende från fem till tre.
-- Tre svar räcker för prisjämförelse och höjer träffsäkerheten för verkstäderna
-- som betalar för leadet. Triggarna pekar på funktionen by name, så en
-- CREATE OR REPLACE räcker – de behöver inte skapas om.
...
-- Samma gräns i RPC:n som drar gratisleads: markera ärendet som fullt vid tre.
...
BEGIN
  ...
  UPDATE public.bike_repair_requests
     SET status = CASE WHEN paid_count >= 3 THEN 'closed_for_responses' ELSE 'has_offers' END,
         updated_at = now()
