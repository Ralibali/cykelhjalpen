  ...
  return (
    <section className="container mx-auto px-4 py-16 max-w-6xl">
      <div className="text-center mb-8">
        <h2 className="font-display text-3xl md:text-4xl font-bold mb-3">{t('Öppna ärenden just nu')}</h2>
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {filtered.map((row, idx) => {
          const when = formatDistanceToNow(new Date(row.created_at), { addSuffix: true, locale: lang === 'en' ? enUS : sv })
          const relativeLabel =
            Date.now() - new Date(row.created_at).getTime() < 24 * 60 * 60 * 1000 ? t('idag') : when
          return (
              ...
              <div className="flex items-start justify-between gap-2">
                ...
                {isUrgent(row.urgency) && (
                  <span className="inline-flex items-center gap-1 shrink-0 text-xs font-semibold px-2 py-0.5 rounded-full bg-accent/15 text-accent">
                    <Flame className="h-3 w-3" /> {t('Bråttom')}
                  </span>
                )}
              </div>
        ...
        >
          <div>
            <p className="text-xs uppercase tracking-wide opacity-80 mb-2">{t('För verkstäder')}</p>
            <p className="font-display font-bold text-lg leading-snug">
              {t('Registrera verkstaden gratis för att se detaljer och svara')}
            </p>
          </div>
          <span className="inline-flex items-center gap-1 text-sm font-semibold">
            {t('Kom igång')} <ArrowRight className="h-4 w-4" />
          </span>
        </Link>
      </div>
    </section>
