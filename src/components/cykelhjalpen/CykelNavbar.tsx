    ...
    <header className={`sticky top-0 z-50 w-full backdrop-blur-md transition-all ${scrolled ? 'bg-background/80 border-b border-border/50 shadow-sm' : 'bg-background/60'}`}>
      <div className="container mx-auto flex h-16 items-center justify-between px-4">
        ...
        <div className="hidden md:flex items-center gap-3">
          ...
          <Button asChild size="sm"><Link to="/skicka-arende">Få prisförslag</Link></Button>
        </div>

        <div className="md:hidden flex items-center gap-2">
          <ThemeToggle />
          <button className="p-2" aria-label={open ? 'Stäng meny' : 'Öppna meny'} onClick={() => setOpen((value) => !value)}>
        ...
        <div className="md:hidden border-t border-border bg-background">
          <div className="container mx-auto px-4 py-4 flex flex-col gap-3">
            ...
            <div className="flex flex-col gap-2 pt-3 border-t border-border">
              ...
              <Button asChild size="sm" onClick={() => setOpen(false)}><Link to="/skicka-arende">Få prisförslag</Link></Button>
            </div>
          </div>
        </div>
      )}
