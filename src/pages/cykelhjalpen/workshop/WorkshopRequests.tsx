...
import { Bike, Loader2, Send, Check, MapPin, RefreshCw, Clock3, Truck, AlertTriangle } from 'lucide-react'
...
const emptyForm: QuoteForm = { message: '', estimated_price_min: '', estimated_price_max: '', estimated_time: '', can_pickup: false }
...
const WorkshopRequests = () => {
  ...
  // Formulärvärden hålls per ärende så inget tappas vid tillfälligt fel.
  ...
  const getForm = (requestId: string): QuoteForm => forms[requestId] ?? emptyForm
  const patchForm = (requestId: string, patch: Partial<QuoteForm>) =>
    setForms((current) => ({ ...current, [requestId]: { ...(current[requestId] ?? emptyForm), ...patch } }))
  ...
  const load = async ({ silent = false }: { silent?: boolean } = {}) => {
    ...
    if (mineError && !silent) toast.error(t('Kunde inte läsa dina tidigare offerter.'))
    if (!mineError) setResponses((mine || []) as ExistingResponse[])
  ...
  const validateOffer = (requestId: string) => {
    const form = getForm(requestId)
    if (form.message.trim().length < 20) return t('Beskriv ditt svar lite mer, minst tjugo tecken.')
    ...
    if ((min !== null && min < 0) || (max !== null && max < 0)) return t('Priset kan inte vara negativt.')
    if (min !== null && max !== null && max < min) return t('Pris till måste vara samma som eller högre än pris från.')
    return null
  }

  // Ett enda klick: validera → skapa och skicka offerten i samma anrop → bekräfta i samma vy.
  const submitOffer = async (requestId: string) => {
    if (submitting) return
    const validationError = validateOffer(requestId)
    if (validationError) {
      setFormErrors((current) => ({ ...current, [requestId]: validationError }))
      toast.error(validationError)
      return
    }

    const form = getForm(requestId)
    setFormErrors((current) => ({ ...current, [requestId]: '' }))
    ...
    if (error || result?.error) {
      ...
      const friendly = isFull
        ? t('Ärendet är fullt – tre verkstäder har redan svarat.')
        : isCompleted
          ? t('Kunden har redan valt en verkstad för det här ärendet.')
          : msg || t('Något gick fel. Dina uppgifter är kvar – försök igen.')

      setSubmitting(null)
      setFormErrors((current) => ({ ...current, [requestId]: friendly }))
      toast.error(t('Kunde inte skicka offerten.'), { description: friendly, duration: 8000 })
      // Uppdatera listan i bakgrunden utan att stänga formuläret eller tappa värden.
      load({ silent: true })
      return
    }

    // Markera direkt i listan – ingen omladdning krävs.
    setResponses((current) => {
      const responseId = String(result?.response_id ?? `${requestId}-sent`)
      const others = current.filter((item) => item.request_id !== requestId)
      return [...others, { id: responseId, request_id: requestId, paid: false, status: 'sent' }]
    })
    setJustSent((current) => ({ ...current, [requestId]: true }))
    setForms((current) => ({ ...current, [requestId]: emptyForm }))
    setSubmitting(null)
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          {requests.map((request) => {
            ...
            const sent = justSent[request.id] || existing?.status === 'sent' || existing?.status === 'won' || (existing?.paid && existing?.status !== 'lost')
            const isSubmitting = submitting === request.id
            const form = getForm(request.id)
            const formError = formErrors[request.id]
            ...
            return (
              <div key={request.id} className="sticker rounded-3xl bg-card p-5 md:p-6">
                <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-4">
                  ...
                  {sent ? (
                    <span className="shrink-0 text-sm flex items-center gap-1.5 rounded-full bg-[hsl(var(--brand-mint)/0.15)] text-[hsl(var(--brand-mint))] font-medium px-3 py-1.5"><Check className="h-4 w-4" /> {t('Offert skickad')}</span>
                  ) : (
                    <Button size="sm" className="shrink-0" onClick={() => toggleOffer(request.id)} disabled={isSubmitting}>
                ...
                {sent && (
                  <div className="mt-4 rounded-2xl border border-[hsl(var(--brand-mint)/0.35)] bg-[hsl(var(--brand-mint)/0.1)] p-4 text-sm">
                    <p className="font-medium flex items-center gap-2"><Check className="h-4 w-4" /> {t('Offerten är skickad till kunden')}</p>
                    <p className="text-muted-foreground mt-1">{t('Du betalar bara om kunden väljer dig. Vi hör av oss så fort kunden svarat.')}</p>
                  </div>
                )}

                {active === request.id && !sent && (
                  <div className="mt-5 space-y-4 rounded-2xl border-2 border-dashed border-border bg-muted/40 p-4 sm:p-5">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      <div className="space-y-1.5"><Label htmlFor={`min-${request.id}`}>{t('Pris från (kr, inkl. moms)')}</Label><Input id={`min-${request.id}`} type="number" inputMode="numeric" min="0" value={form.estimated_price_min} onChange={(event) => patchForm(request.id, { estimated_price_min: event.target.value })} className="rounded-xl border-2 bg-background" /></div>
                      <div className="space-y-1.5"><Label htmlFor={`max-${request.id}`}>{t('Pris till (kr, inkl. moms)')}</Label><Input id={`max-${request.id}`} type="number" inputMode="numeric" min="0" value={form.estimated_price_max} onChange={(event) => patchForm(request.id, { estimated_price_max: event.target.value })} className="rounded-xl border-2 bg-background" /></div>
                    </div>
                    <div className="space-y-1.5"><Label htmlFor={`time-${request.id}`}>{t('Beräknad tid')}</Label><Input id={`time-${request.id}`} value={form.estimated_time} onChange={(event) => patchForm(request.id, { estimated_time: event.target.value })} placeholder={t('Exempel: cirka en timme eller två arbetsdagar')} className="rounded-xl border-2 bg-background" /></div>
                    <div className="space-y-1.5"><Label htmlFor={`message-${request.id}`}>{t('Meddelande till kunden')}</Label><Textarea id={`message-${request.id}`} rows={4} value={form.message} onChange={(event) => patchForm(request.id, { message: event.target.value })} placeholder={t('Beskriv vad ni rekommenderar, vad priset omfattar och när ni kan ta emot cykeln.')} className="rounded-xl border-2 bg-background" /></div>
                    ...
                    <label className="flex items-center gap-2 text-sm font-medium"><input type="checkbox" className="h-4 w-4 rounded" checked={form.can_pickup} onChange={(event) => patchForm(request.id, { can_pickup: event.target.checked })} /> {t('Vi kan hämta cykeln')}</label>
                    ...
                    <Button onClick={() => submitOffer(request.id)} disabled={isSubmitting || Boolean(submitting)} className="w-full rounded-xl cta-playful bg-accent text-accent-foreground hover:bg-accent/90 h-12">
