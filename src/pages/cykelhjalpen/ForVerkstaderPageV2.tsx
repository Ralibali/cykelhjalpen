...
const ForVerkstaderPageV2 = () => {
  const { lang } = useLanguage()
  const pageSeo = usePageSeo('/for-cykelverkstader')
  const text = (sv: string, en: string) => lang === 'en' ? en : sv
  const registerHref = '/registrera/verkstad?stad=linkoping'
  const trackCta = (placement: string) => trackClick('workshop_partner_cta', 'Founding Partner', { placement, city: 'Linköping' })
  ...
  return (
    <div className="min-h-screen bg-background">
      <Helmet>
        <title>{text('Få fler lokala kunder till din cykelverkstad | Cykelhjälpen', 'Get more local customers for your bike shop | Cykelhjälpen')}</title>
        <meta name="description" content={text('Bli Founding Partner i Cykelhjälpen. Ingen månadsavgift, två första vunna kunderna gratis och full frihet att välja vilka jobb ni vill svara på.', 'Become a Founding Partner with Cykelhjälpen. No monthly fee, your first two won customers are free, and you choose which jobs to respond to.')} />
      ...
      <main>
        <section className="bg-hero-gradient">
          <div className="container mx-auto px-4 py-14 md:py-20 max-w-6xl grid lg:grid-cols-[1fr_.9fr] gap-10 items-center">
            <div>
              <span className="inline-flex items-center gap-2 rounded-full bg-accent/15 text-accent px-3.5 py-1.5 text-sm font-semibold mb-6"><Sparkles className="h-4 w-4" /> Founding Partner</span>
              <h1 className="font-display text-4xl md:text-6xl tracking-tight leading-[1.03]">{text('Få in fler lokala cykeljobb', 'Get more local bike jobs')} <span className="text-accent italic">{text('utan månadsavgift.', 'with no monthly fee.')}</span></h1>
              ...
              <div className="mt-7 rounded-2xl border-2 border-primary/30 bg-background/80 p-5 max-w-2xl"><p className="font-semibold flex items-center gap-2"><MapPin className="h-4 w-4 text-primary" />{text('Linköping är vår prioriterade stad just nu', 'Linköping is our priority city right now')}</p><p className="text-sm text-muted-foreground mt-2">{text('Målet är att bygga ett litet, starkt nätverk av 5–8 aktiva partnerverkstäder.', 'The goal is to build a small, strong network of 5–8 active partner bike shops.')}</p></div>
              <Button asChild size="lg" className="mt-8 rounded-full h-14 px-8"><Link to={registerHref} onClick={() => trackCta('hero')}>{text('Bli Founding Partner', 'Become a Founding Partner')} <ArrowRight className="h-4 w-4 ml-2" /></Link></Button>
        ...
        <section className="py-20"><div className="container mx-auto px-4 max-w-4xl"><div className="sticker rounded-[2rem] bg-[hsl(var(--brand-dark))] text-background p-9 md:p-14 text-center"><Store className="h-11 w-11 mx-auto text-[hsl(var(--brand-sun))] mb-5" /><h2 className="font-display text-4xl md:text-5xl">{text('Var med från början i Linköping', 'Join from the start in Linköping')}</h2><p className="mt-5 text-background/70 text-lg max-w-2xl mx-auto">{text('Registreringen är gratis. Testa riktiga kundärenden och avgör själv om Cykelhjälpen skapar värde för verkstaden.', 'Registration is free. Try real customer requests and decide whether Cykelhjälpen creates value for your shop.')}</p><Button asChild size="lg" className="mt-8 rounded-full h-14 px-10 bg-accent text-accent-foreground"><Link to={registerHref} onClick={() => trackCta('bottom')}>{text('Registrera verkstaden gratis', 'Register your shop for free')} <ArrowRight className="h-4 w-4 ml-2" /></Link></Button></div></div></section>
      </main>
      <CykelFooter />
    </div>
  )
...
export default ForVerkstaderPageV2
