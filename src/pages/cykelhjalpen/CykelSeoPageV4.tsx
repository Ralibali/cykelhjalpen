...
type PriceRow = { repair_category: string; sample_count: number; price_low: number; price_high: number; price_typical: number }
...
const PriceStatsTable = ({ city, lang }: { city: CykelCityName; lang: Lang }) => {
  ...
  const verifiedRows = (data ?? []).filter((row) => row.sample_count >= 3)
  const isFallback = verifiedRows.length === 0
  const rows = isFallback ? FALLBACK_PRICES : verifiedRows

  return (
    <section className="my-10" aria-labelledby="prisstatistik-rubrik">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between mb-4">
        <div>
          <p className="text-xs uppercase tracking-[.18em] text-accent font-semibold">{isFallback ? text('Riktpriser', 'Guide prices') : text('Verifierat underlag', 'Verified data')}</p>
          <h2 id="prisstatistik-rubrik" className="font-display text-2xl font-bold mt-1">{text('Prisunderlag per reparationstyp', 'Price data by repair type')}</h2>
        </div>
        {!isFallback && <span className="text-xs rounded-full border border-border bg-muted/60 px-3 py-1.5 text-muted-foreground">{text('Minst 3 offerter per rad', 'At least 3 quotes per row')}</span>}
      </div>
      <div className="overflow-x-auto sticker bg-card p-4 rounded-2xl">
        <table className="w-full text-sm">
          <thead><tr className="text-left border-b"><th className="py-2 pr-4">{text('Reparationstyp', 'Repair type')}</th><th className="py-2 pr-4">{text('Prisspann', 'Price range')}</th><th className="py-2 pr-4">{text('Typiskt pris', 'Typical price')}</th><th className="py-2">{text('Underlag', 'Source')}</th></tr></thead>
          <tbody>{rows.map((row) => <tr key={row.repair_category} className="border-b last:border-0"><td className="py-2 pr-4 font-medium">{lang === 'en' ? EN_PRICE_LABELS[row.repair_category] ?? row.repair_category : row.repair_category}</td><td className="py-2 pr-4">{row.price_low}–{row.price_high} kr</td><td className="py-2 pr-4">{text(`cirka ${row.price_typical} kr`, `about SEK ${row.price_typical}`)}</td><td className="py-2 text-muted-foreground">{isFallback ? text('riktpris', 'guide price') : text(`${row.sample_count} offerter`, `${row.sample_count} quotes`)}</td></tr>)}</tbody>
        </table>
      </div>
      <p className="text-sm text-muted-foreground mt-3 leading-relaxed">{isFallback
        ? text('Generella riktpriser – inte Cykelhjälpen-statistik eller bindande offerter. Faktisk statistik visas först när minst tre relevanta offerter finns i underlaget.', 'General guide prices — not Cykelhjälpen statistics or binding quotes. Actual statistics are shown only when at least three relevant quotes are available.')
        : text(`Statistiken bygger på offerter via Cykelhjälpen och är inte avgränsad till ${city}. För pris på just din cykel i ${city}, skicka ett kostnadsfritt ärende.`, `The statistics are based on quotes submitted through Cykelhjälpen and are not limited to ${city}. For a price on your bike in ${city}, send a free request.`)}</p>
    </section>
  )
}
...
const CykelSeoPageV4 = () => {
  ...
  const seoDescription = page.variant === 'price-stats'
    ? text(`Se verifierad prisstatistik från Cykelhjälpen när minst tre offerter finns i underlaget, annars tydligt märkta riktpriser. Skicka ett ärende för lokalt pris i ${city}.`, `See verified Cykelhjälpen price statistics when at least three quotes are available; otherwise clearly labelled guide prices are shown. Send a request for a local price in ${city}.`)
    : page.description
  ...
  return <div className="min-h-screen bg-background"><Helmet><title>{page.title}</title><meta name="description" content={seoDescription} /><meta name="robots" content="index, follow, max-image-preview:large, max-snippet:-1, max-video-preview:-1" /><link rel="canonical" href={canonical} /><link rel="alternate" hrefLang="sv" href={svUrl} /><link rel="alternate" hrefLang="en" href={enUrl} /><link rel="alternate" hrefLang="x-default" href={svUrl} /><meta property="og:type" content="website" /><meta property="og:locale" content={lang === 'en' ? 'en_US' : 'sv_SE'} /><meta property="og:site_name" content="Cykelhjälpen" /><meta property="og:title" content={page.title} /><meta property="og:description" content={seoDescription} /><meta property="og:url" content={canonical} /><meta property="og:image" content={`https://cykelhjalpen.se${ogImage}`} /><meta name="twitter:card" content="summary_large_image" /><script type="application/ld+json">{JSON.stringify(jsonLd)}</script></Helmet><CykelNavbar /><main className="container mx-auto px-4 py-12 max-w-3xl"><article>
    ...
    {page.variant === 'price-stats' && <PriceStatsTable city={city} lang={lang} />}
