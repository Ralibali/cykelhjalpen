...
const AdminUsers = () => {
  ...
  const handleExport = () => {
    exportCsv(filtered.map(u => ({
      [t('Namn')]: u.full_name || '–',
      [t('E-post')]: u.email || '–',
      [t('Företag')]: u.company_name || '–',
      [t('Roll')]: u.role,
      [t('Stad')]: u.city || '–',
      [t('Telefon')]: u.phone || '–',
      BankID: u.is_bankid_verified ? t('Ja') : t('Nej'),
      [t('Registrerad')]: u.created_at ? new Date(u.created_at).toLocaleDateString('sv-SE') : '',
    })), 'anvandare')
  }
  ...
  return (
    <AdminLayout>
      <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
        <h1 className="font-display text-2xl font-bold">{t('Användare')}</h1>
        <div className="flex items-center gap-3">
          ...
          <div className="relative w-56">
            ...
            <Input placeholder={t('Sök användare...')} value={search} onChange={e => setSearch(e.target.value)} className="pl-10 rounded-xl" />
          </div>
      ...
      <div className="bg-card rounded-xl border overflow-hidden overflow-x-auto">
        <table className="w-full min-w-[760px] text-sm">
          <thead><tr className="border-b bg-muted/50">
            <th className="text-left p-3 font-medium">{t('Namn')}</th>
            <th className="text-left p-3 font-medium">{t('E-post')}</th>
            <th className="text-left p-3 font-medium">{t('Företag')}</th>
            <th className="text-left p-3 font-medium">{t('Roll')}</th>
            <th className="text-left p-3 font-medium">{t('Verifierad')}</th>
            <th className="text-left p-3 font-medium">{t('Registrerad')}</th>
          </tr></thead>
          <tbody>
            {filtered.map(u => (
              <tr key={u.id} className="border-b last:border-0 hover:bg-muted/30">
                ...
                <td className="p-3">
                  <div className="flex gap-1">
                    ...
                    {u.is_phone_verified && <span className="text-[10px] bg-blue-100 text-blue-700 rounded-full px-1.5 py-0.5">{t('Tel')}</span>}
        ...
        {filtered.length === 0 && <p className="p-6 text-center text-muted-foreground">{t('Inga användare hittades.')}</p>}
      </div>
    </AdminLayout>
  )
}
